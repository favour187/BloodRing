...
public class MapGenerator : MonoBehaviour
{
    ...
    public void GenerateMap(string mapChoice = "IslaVerde")
    {
        ...
        // ── MODERN 3D BATTLE ROYALE INTEGRATION ──────────────────
        // 1. Blended Terrain Texture Painting
        if (TerrainTexturePainter.Instance == null) gameObject.AddComponent<TerrainTexturePainter>();
        TerrainTexturePainter.Instance.PaintTerrain(currentMapData, terrain);

        // 2. Spatial Pathfinding Grid
        if (NavGridSystem.Instance == null) gameObject.AddComponent<NavGridSystem>();
        NavGridSystem.Instance.BuildNavGrid(currentMapData);

        // 3. Tactical Minimap Radar Generation
        if (MinimapGenerator.Instance == null) gameObject.AddComponent<MinimapGenerator>();
        MinimapGenerator.Instance.GenerateMinimapTexture(currentMapData);
